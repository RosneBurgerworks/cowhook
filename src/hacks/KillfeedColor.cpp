        ...
        if (!displayed_players.empty())
        {
            ...
            for (auto &player : displayed_players)
            {
                ...
                if (player_color != colors::empty)
                {
                    ...
                    static float rainbow_speed = 0.05f; // Adjust this value for a slower rainbow effect
                    static int start_time = g_GlobalVars->tickcount;
                    float time = (g_GlobalVars->tickcount - start_time) * rainbow_speed;
                    int r = static_cast<int>((0.5f + 0.5f * sinf(time)) * 255);
                    int g = static_cast<int>((0.5f + 0.5f * sinf(time + 2.0f)) * 255);
                    int b = static_cast<int>((0.5f + 0.5f * sinf(time + 4.0f)) * 255);
                    draw_clr.SetColor(r, g, b, 255);
                }
