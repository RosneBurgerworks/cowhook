...
static void doAutoZoom(bool target_found, CachedEntity *target)
{
    ...
    // Thanks sydney615 for this lol
    // Minigun spun up handler / Rev Dist
    if (auto_spin_up && g_pLocalPlayer->weapon()->m_iClassID() == CL_CLASS(CTFMinigun))
    {
        if ((target_found || isIdle || nearest.second <= *rev_distance)) 
            zoomTime.update();
...
// Uses getNearestPlayerDistance to get the position of the player and pre-scopes 
